    ...
    <nav className={`sticky top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-white/70 backdrop-blur-md shadow-sm py-2" : "bg-slate-50 py-4"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex gap-8 items-center">
            ...
            <Link href="/add-tutor" className="font-medium text-slate-700 hover:text-blue-600 transition-colors">Add Tutors</Link>
            <Link href="/my-tutor" className="font-medium text-slate-700 hover:text-blue-600 transition-colors">My Tutors</Link>
            <Link href="/dashboard" className="font-medium text-slate-700 hover:text-blue-600 transition-colors">My Booked Session</Link>
          </div>
            ...
            <div className="relative group">
              <button className="flex items-center gap-3 p-1 rounded-full hover:bg-muted transition-colors border border-transparent hover:border-border">
                ...
                <div className="text-left hidden lg:block">
                  <p className="text-sm font-bold truncate max-w-25">Farhan</p>
              ...
              <div className="absolute right-0 top-12 w-56 bg-white border border-slate-200 rounded-2xl shadow-2xl hidden group-hover:flex flex-col py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="px-4 py-3 border-b border-slate-100">
                  ...
                  <p className="text-xs truncate text-slate-500">farhan@gmail.com</p>
                </div>
      ...
      {isMenuOpen && (
        <div className="md:hidden px-4 pt-2 pb-6 space-y-2 bg-white border-b border-slate-200 animate-in slide-in-from-top duration-300">
          ...
          <Link href="/add-course" className="block px-4 py-3 text-base font-medium text-slate-900 hover:bg-slate-50 rounded-xl">Add Tutors</Link>
          <Link href="/add-course" className="block px-4 py-3 text-base font-medium text-slate-900 hover:bg-slate-50 rounded-xl">My Tutors</Link>
          <Link href="/dashboard" className="block px-4 py-3 text-base font-medium text-slate-900 hover:bg-slate-50 rounded-xl">My Booked Sessions</Link>
